...
// FunctionCallUtils.getFunctionCallback(reactThinkResult.getAction());
//                    String response = "";
//                    if (functionCallback != null) {
//                        response = functionCallback.call(reactThinkResult.getActionInput(), new
// ToolContext(options.getToolContext() == null ? new HashMap<>() : options.getToolContext()));
//                    }
//                    reactThinkResult.setActionOutput(response);
//                    reActThinkResults.add(reactThinkResult);
//                    output = reactThinkResult.getOriginText() + "\nObservation: " +
// reactThinkResult.getActionOutput();
//                } else {
//                    output += "\n" + reactThinkResult.getOriginText();
//                    reactThinkChainsResult = parseFinalAnswer(reActThinkResults,
// reactThinkResult.getOriginText(), output);
//                    break;
//                }
//            }
//            return ChatResponse.builder().withMetadata(META_KEY, reactThinkChainsResult)
//                    .withGenerations(List.of(new Generation(new
// AssistantMessage(reactThinkChainsResult.getFinalAnswer()))))
//                    .build();
//        }
...
// chatResponse.getResult().getOutput().getMetadata()), chatResponse.getResult().getMetadata())),
...
// options)).getResult().getOutput().getContent();
//    }
//
...
//                    return answer.get().contains(stop) ||
// chatResponse.getResult().getMetadata().getFinishReason().equals("stop");
//                })
...
//                        String[] split =
// chatResponse.getResult().getOutput().getContent().split(stop);
//                        return Flux.just(copyResponse(split.length > 1 ? split[1] : "",
// chatResponse));
//                    }
...
//                        String response = "";
//                        if (functionCallback != null) {
//                            response = functionCallback.call(reactThinkResult.getActionInput(),
// new ToolContext(options.getToolContext() == null ? new HashMap<>() : options.getToolContext()));
//                        }
//                        reactThinkResult.setActionOutput(response);
...
//        Set<String> functionsForThisRequest = new
// HashSet<>(FunctionCallUtils.findFunctions(options));
//        List<FunctionCallback> functionCallbacks =
// FunctionCallUtils.getFunctionCallback(functionsForThisRequest);
...
//                                "parameters",
// functionCallback.getInputTypeSchema())).getContent())
...
//                Map<String, Object> params = Map.of("query", content, "tools_text", tools,
// "tools_name_text", names);
...
//                {im_start}{type}
//                {query}{im_end}""");
//        return queryTemplate.createMessage(Map.of("im_start", imStart, "im_end", imEnd, "query",
...
//            String thought = text.substring(thoughtIndex + "Thought:".length(),
// actionIndex).trim();
//            String action = text.substring(actionIndex + "\nAction:".length(),
// actionInputIndex).trim();
//            String actionInput = text.substring(actionInputIndex + "\nAction Input:".length(),
// observationIndex).trim();
//            return new ReActThinkResult(thought, action, actionInput, text.substring(0,
// observationIndex), "");
//        } else {
...
//    }
//
//    private ReActThinkChainsResult parseFinalAnswer(List<ReActThinkResult> reActThinkResults,
// String text, String originText) {
...
//            String thought = text.substring(thoughtIndex + "Thought:".length(),
// finalAnswerIndex).trim();
//            String finalAnswer = text.substring(finalAnswerIndex + "\nFinal
// Answer:".length()).trim();
//            return new ReActThinkChainsResult(reActThinkResults, thought, finalAnswer,
// originText);
//        }
