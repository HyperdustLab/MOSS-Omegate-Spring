...
public class OllamaApi {
   ...
   */
  public OllamaApi(
      String baseUrl, RestClient.Builder restClientBuilder, WebClient.Builder webClientBuilder) {
   ...
   * (controlled by the 'stream' request property), so there will be a series of responses. The
  ...
  public Flux<ChatResponse> streamingChat(ChatRequest chatRequest) {
    ...
    logger.info("Streaming chat request: " + StrUtil.join("\n", chatRequest.messages()));
  ...
  private static class OllamaResponseErrorHandler implements ResponseErrorHandler {
    ...
    public void handleError(ClientHttpResponse response) throws IOException {
      if (response.getStatusCode().isError()) {
        ...
        String message =
            StreamUtils.copyToString(response.getBody(), java.nio.charset.StandardCharsets.UTF_8);
   ...
   * @param format (optional) The format to return the response in. Currently, the only accepted
   *     value is "json".
   * @param options (optional) additional model parameters listed in the documentation for the Model
   *     file such as temperature.
   * @param system (optional) system prompt to (overrides what is defined in the Model file).
   * @param template (optional) the full prompt or prompt template (overrides what is defined in the
   *     Model file).
   * @param context the context parameter returned from a previous request to /generate, this can be
   *     used to keep a short conversational memory.
   * @param stream (optional) if false the response will be returned as a single response object,
   *     rather than a stream of objects.
   * @param raw (optional) if true no formatting will be applied to the prompt and no context will
   *     be returned. You may choose to use the raw parameter if you are specifying a full templated
   *     prompt in your request to the API, and are managing history yourself.
   * @param images (optional) a list of base64-encoded images (for multimodal models such as llava).
   * @param keepAlive (optional) controls how long the model will stay loaded into memory following
    ...
    public static class Builder {
      ...
      public GenerateRequest build() {
        return new GenerateRequest(
            model, prompt, format, options, system, template, context, stream, raw, images,
            keepAlive);
      }
    }
  }
  ...
  /**
   * The response object returned from the /generate endpoint. To calculate how fast the response is
   * generated in tokens per second (token/s), divide eval_count / eval_duration.
   *
   * @param model The model used for completion.
   * @param createdAt When the request was made.
   * @param response The completion response. Empty if the response was streamed, if not streamed,
   *     this will contain the full response
   * @param done Whether this is the final response. If true, this response may be followed by
   *     another response with the following, additional fields: context, prompt_eval_count,
   *     prompt_eval_duration, eval_count, eval_duration.
   * @param context Encoding of the conversation used in this response, this can be sent in the next
   *     request to keep a conversational memory.
   ...
   * @param images The list of base64-encoded images to send with the message. Requires multimodal
  ...
  public record Message(
     ...
     */
    @JsonInclude(Include.NON_NULL)
    public record ToolCall(@JsonProperty("function") ToolCallFunction function) {}
   ...
   * Chat request object.
   *
   * @param model The model to use for completion. It should be a name familiar to Ollama from the
   *     <a href="https://ollama.com/library">Library</a>.
   * @param messages The list of messages in the chat. This can be used to keep a chat memory.
   * @param stream Whether to stream the response. If false, the response will be returned as a
   *     single response object rather than a stream of objects.
   * @param format The format to return the response in. Currently, the only accepted value is
   *     "json".
   * @param keepAlive Controls how long the model will stay loaded into memory following this
   *     request (default: 5m).
   * @param tools List of tools the model has access to.
   * @param options Model-specific options. For example, "temperature" can be set through this
   *     field, if the model supports it. You can use the {@link OllamaOptions} builder to create
   *     the options then {@link OllamaOptions#toMap()} to convert the options into a map.
   * @see <a href=
   ...
   * @param done Whether this is the final response. For streaming response only the last message is
   *     marked as done. If true, this response may be followed by another response with the
   *     following, additional fields: context, prompt_eval_count, prompt_eval_duration, eval_count,
   *     eval_duration.
   * @param totalDuration Time spent generating the response.
   * @param loadDuration Time spent loading the model.
   ...
   * @param keepAlive Controls how long the model will stay loaded into memory following the request
   *     (default: 5m).
   * @param options Additional model parameters listed in the documentation for the
   * @param truncate Truncates the end of each input to fit within context length. Returns error if
  ...
  @Deprecated(since = "1.0.0-M2", forRemoval = true)
  @JsonInclude(Include.NON_NULL)
  public record EmbeddingResponse(@JsonProperty("embedding") List<Float> embedding) {}

  /**
   * The response object returned from the /embedding endpoint.
   *
   * @param model The model used for generating the embeddings.
   * @param embeddings The list of embeddings generated from the model. Each embedding (list of
   *     doubles) corresponds to a single input text.
   */
  @JsonInclude(Include.NON_NULL)
  public record EmbeddingsResponse(
      @JsonProperty("model") String model, @JsonProperty("embeddings") List<float[]> embeddings) {}
}
// @formatter:on
